...
function setupTextScrambleEffect() {
    ...
    style.textContent = `
        [data-gallery-text="title"], [data-gallery-text="author"] {
            visibility: visible;
            opacity: 1;
        }
        ...
        @keyframes textScramble {
            0%, 20% { content: '_'; }
            40% { content: attr(data-char-1); }
            60% { content: attr(data-char-2); }
            80% { content: attr(data-char-3); }
            100% { content: attr(data-char); }
        }
        
        .text-scramble-active .text-scramble-char::after {
            animation: textScramble 0.5s calc(var(--char-index, 0) * 0.04s) forwards;
        }
    ...
    function prepareTextForScramble(element) {
        ...
        // Генерируем случайные символы для scrambling
        const randomChars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()_+-=[]{}|;:,./<>?';
        
        // Создаем HTML для каждого символа
        const html = chars.map((char, index) => {
            ...
            return `<span class="text-scramble-char" 
                        ...
                        data-char-2="${char2}" 
                        data-char-3="${char3}" 
                        style="--char-index: ${index};">${char}</span>`;
        }).join('');
...
void main() {
    ...
    // Центрируем позицию мыши относительно середины и инвертируем направление
    float mouseX = -(uMousePosition.x - 0.5);
    float mouseInfluence = 0.3;
    float mouseRotation = mouseX * mouseInfluence * PI;
...
class CircularGallery {
    constructor(canvas) {
        ...
        style.textContent = `
            ...
            /* Добавляем стили для управления z-index свайпера */
            [data-gallery="swiper"] {
                transition: z-index 0.2s linear;
            }
            [data-gallery="swiper"].initial-animation {
                z-index: 0 !important;
            }
        `;
        document.head.appendChild(style);
    ...
    handleMouseMove(event) {
        // Игнорируем движение мыши во время начальной анимации
        if (this.isInitialAnimationPlaying) return;
        
        const x = event.clientX / window.innerWidth;
        const y = 1 - event.clientY / window.innerHeight;
        
        const dx = Math.abs(x - this.targetMousePosition.x);
        const dy = Math.abs(y - this.targetMousePosition.y);
        
        if (dx > this.mouseMoveTolerance || dy > this.mouseMoveTolerance) {
    ...
    playInitialAnimation() {
        ...
        // Устанавливаем z-index свайпера в 0 на время анимации
        const swiperElement = document.querySelector('[data-gallery="swiper"]');
        if (swiperElement) {
            swiperElement.classList.add('initial-animation');
        }
            ...
            .fromTo(this.params, 
                ...
                },
                {
                    ...
                    onComplete: () => {
                        ...
                        // Восстанавливаем z-index свайпера
                        const swiperElement = document.querySelector('[data-gallery="swiper"]');
                        if (swiperElement) {
                            // Используем setTimeout, чтобы дать время для завершения анимации
                            setTimeout(() => {
                                swiperElement.classList.remove('initial-animation');
                            }, 50);
                        }
