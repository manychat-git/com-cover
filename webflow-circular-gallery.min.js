...
class CircularGallery {
    constructor(canvas) {
        ...
        // Заменяем отдельные текстуры на объект с тремя текстурами
        this.textures = {
            current: null,
            next: null,
            previous: null
    ...
    async updateImage(newImage, skipAnimation = false) {
        ...
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.textures.next);
        this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, this.gl.RGBA, this.gl.UNSIGNED_BYTE, newImage);
        ...
        gsap.timeline()
            .to(this.params, {
                ...
                onComplete: () => {
                    // Ротация текстур
                    const temp = this.textures.previous;
                    this.textures.previous = this.textures.current;
                    this.textures.current = this.textures.next;
                    this.textures.next = temp;

                    // Обновляем привязки текстур
                    this.gl.activeTexture(this.gl.TEXTURE0);
                    this.gl.bindTexture(this.gl.TEXTURE_2D, this.textures.current);
    ...
    initWebGL() {
        ...
        // Создаем три текстуры вместо двух
        this.textures.current = this.gl.createTexture();
        this.textures.next = this.gl.createTexture();
        this.textures.previous = this.gl.createTexture();

        // Инициализируем все три текстуры
        [this.textures.current, this.textures.next, this.textures.previous].forEach(texture => {
    ...
    render(time) {
        ...
        // Обновляем привязки текстур перед рендерингом
        this.gl.activeTexture(this.gl.TEXTURE0);
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.textures.current);
